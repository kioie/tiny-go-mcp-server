...
> Minimal Go library on the official MCP go-sdk — stdio, streamable HTTP, legacy SSE; tools, resources, prompts; struct-derived JSON Schema; ~5MB static binaries. Reference server on Glama and MCP Registry (`io.github.kioie/tiny-go-mcp` v1.2.0).
...
Requires Docker on the client machine (bundle runs `ghcr.io/kioie/tiny-go-mcp:1.2.0`). Alternative: connect GitHub at https://smithery.ai/new.
